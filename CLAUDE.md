...
3. docs/ticket-reihenfolge.md               # nächstes Ticket = oberstes freies Item der Board-Reihenfolge (gh project item-list 1 --owner fluffels --format json --limit 800, NICHT nachsortieren; braucht read:project-Scope)
...
| [`src/core/keybindings.ts`](src/core/keybindings.ts) | pure Domäne | Umbelegbare Aktionstasten (#232): Aktions-Liste + Default-Belegung (R/F/L/B) + Belegungsregeln (`isAssignableKey`/`resolveAction`/`findConflict`/`sanitizeKeybindings`); persistiert als `settings.keys`, angewandt im Tastatur-Dispatch (main.ts) + Menü-UI (ui/overlay.ts). |
...
| [`src/store/versioning.ts`](src/store/versioning.ts) | Persistenz | Save-Format-Versionierung (#350/#510): `{ v, data }`-Hülle, Migrationskette (`CURRENT_SAVE_VERSION`), `readState`/`writeState`/`migrateParsed` + Backup-vor-Überschreiben (#515). Migrationen sind meist strukturelle No-ops (Härtung läuft in `sanitizeState`), außer wenn ein Alt-Wert wirklich umgedeutet werden muss (v6→v7 hebt das additive Abkürzungs-Array auf die Komfort-Kauf-Mechanik, #574). |
...
| 🥇 Nächstes Ticket (Auswahl-Regel) | [docs/ticket-reihenfolge.md](docs/ticket-reihenfolge.md) – oberstes freies Item der manuellen Board-Reihenfolge (kein `Prio`-Feld mehr seit #747, keine Datei-Pflege seit #627) |
...
- **Welches Ticket nehme ich?** Das **oberste freie Item in der manuellen Board-Reihenfolge** (die Maintainerin zieht die Tickets in View 1 wie eine Warteschlange in Position), gelesen aus dem Project-Board (`gh project item-list 1 --owner fluffels --format json --limit 800`, in Board-Reihenfolge, NICHT nachsortieren; braucht `read:project`-Scope; frei = kein Assignee, kein offener PR/Branch/Worktree, nicht `status:zurückgestellt`, kein offener Blocker). Nur **dieses eine** Kandidaten-Ticket prüfen, nicht die ganze Liste. Sofort self-assignen. Fertig gefilterter Befehl: [Ticket-Auswahl](docs/ticket-reihenfolge.md). → [AGENTS.md › Wo die TODOs leben](AGENTS.md#wo-die-todos-leben).
