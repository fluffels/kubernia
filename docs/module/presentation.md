...
| `src/scenes/RegionScene.ts` | **EINE** datengetriebene Szene für alle Nachbar-Regionen (#427, erbt `IslandScene`): die gemeinsamen Methoden (`create`/`update`/`renderGround`/`scatterDecor`/`nearestNpc`/`isSolidAt`/…) liegen hier einmal, gesteuert über eine `RegionConfig`. Ersetzt die drei früheren ~90%-identischen Insel-Szenen-Klassen (ArchipelScene/LighthouseScene/WarehouseScene). |
| `src/scenes/regions.ts` | Die `RegionConfig`-Liste (`REGION_CONFIGS`, #427): Archipel #92/Leuchtturm #111/Lager #124 als **Daten** (Geometrie-Builder pur aus `src/world/regions/archipel.ts`/`lighthouse.ts`/`warehouse.ts`, Boden-Deko-Bänder, Titel/Hinweis, Rück-Warp/-Marker) + ein optionaler `decorate`-Hook je Region für die **echte Sondermechanik** (Archipel-Bäume + Quest-Trigger-Statue, Leuchtturm-Lichtkegel, Lager-Güter-Hitboxen). Eine neue Standard-Region = ein weiterer Eintrag hier, keine neue Klasse (Kern-AK von #415). |
...
| `src/scenes/worldscene/worldconfig.ts` | Datengetriebene Karten-Config (`WorldSceneConfig`/`WORLD_CONFIGS`, #863): hafen-spezifischer Aufbau (Objekte/Deko/Plattformen/Auszahlung/Sonder-NPCs) als Datensatz — analog zum `REGION_CONFIGS`-Muster; WorldScene.create() ruft nur noch den generischen Schritt auf. |
...
| `src/scenes/worldscene/terrain.ts` | HAFEN-spezifische Welt: sichtbare Objekte/Gebäude/Warpschilder setzen (`placeHarborObjects`), Türen begehbar schneiden (`carveDoors`/`makeDoor`), Wang-Autotile-Boden zeichnen (`renderGround`, dünner Wrapper um `worldscene/renderground.ts`). Der gemeinsame Terrain-Schritt liegt seit #425 in `mapterrain.ts`; die datengetriebene Region-Szenerie der Nachbar-Regionen wurde mit #427 (RegionScene + `regions.ts`) umgesetzt. |
| `src/scenes/worldscene/groundtiles.ts` | **Phaser-freie SSOT der Wang-Autotile-Kachelwahl (#958):** `groundTile` entscheidet je Kachel Sheet+Frame (Wasser-Rand/Dock/Stein-Kai/Weg/Gras) aus dem Boden-Raster; Helfer `groundLevel`/`cornerCode`/`touchesLevel`/`edgeMaterial`/`isWaterCell` + die SSOT der `WANG`-Tabelle (vorher dupliziert in `scenes/shared.ts`, das sie seither von hier re-exportiert). Vorher fast-identisch dupliziert in `terrain.ts` (Hafen) und `RegionScene.ts` (Regionen), inkl. einer echten Divergenz beim Weg-Eck-Code (`^15`, #866/#914 grass-dirt-Tileset-Tausch) – hier auf den EINEN korrekten Transform reconciled; node-testbar (`test/worldscene-groundtiles.test.ts`). |
| `src/scenes/worldscene/renderground.ts` | Dünne Phaser-Schicht über `groundtiles.ts`: `renderGround(host, opts)` stempelt das Kachel-Raster in eine RenderTexture + optionale Hafen-Atmosphäre (Wasser-Hintergrund, Schaum-Sparkles, Küstenwellen) über Optionen statt einer zweiten Kopie je Aufrufer (#958). |
