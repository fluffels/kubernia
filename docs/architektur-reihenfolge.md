...
|---|-------|--------|--------------|---------------------------|
| 1 | Skalierungs-Enabler | **#426** | Warps datengetrieben: Daten-Liste statt N `enterXxx()` + N `*Armed`-Flags | Kind von #415. Erfüllt die #415-AK „Warps datengesteuert; kein Pro-Region-Flag-Boilerplate". Zieht den gemeinsamen Insel→Welt-Rück-Warp als Vorarbeit für #427 zusammen. |
| 2 | Skalierungs-Enabler | **#427** | Drei Insel-Szenen (Archipel/Leuchtturm/Lager) zu EINER datengetriebenen `RegionScene` vereinheitlichen | Kind von #415, **größter Brocken**. Erfüllt die Kern-AK „neue Standard-Region über Daten, nicht über eine Szenen-Klasse". **Nach #426** (gemeinsamer Rück-Warp). Greift dabei die in #425 angelegte Hafen-Szenerie (`placeHarborObjects`) als datengetriebene Region-Szenerie. |
| 3 | Skalierungs-Enabler | **#428** | `MapId`-Union aus der Registry ableiten (keine hartcodierte Union mehr) | Kind von #415, Abschluss-Härtung. **Nach #425 (✓).** `prio:niedrig`. |
