...
- Bleibt ein Agent an derselben roten Prüfung auf seinem eigenen Ticket hängen (versteht die Ursache nicht, der Fix greift wiederholt nicht), gibt es keinen technisch erzwungenen Stopp. Das **Festgefahren-Protokoll** (#710, AGENTS.md: nach drei erfolglosen Fix-Versuchen ein konsolidierter Entscheidungs-Kommentar + Label `status:festgefahren` statt endlosem Weiterversuchen) ist eine **Verhaltensregel, kein Gate** — sie hält nur, wenn der Agent sie befolgt. Ein echter, erzwungener Stopp bräuchte einen Zähler außerhalb der Agenten-Session (z.B. ein Wrapper-Skript, das fehlgeschlagene `gh pr checks`-Läufe zählt) und existiert bisher nicht.
