...
- **Kontext-Ökonomie des Reviews: einmal beschaffen, nicht pro Lens (#1034).** Gemessen an Ticket #1021 kostete ein einzelnes Ticket **1,03 Mio Subagent-Tokens — 85 % davon im Review** (5 Lens-Pässe über 2 Runden). Die Befunde rechtfertigten den Review inhaltlich (Runde 1 fand einen echten Blocker), der teuerste Posten war aber **Beschaffung, nicht Analyse**: derselbe Diff fünfmal per `git diff` erhoben, die geänderten Dateien fünfmal vollständig gelesen, und `AGENTS.md` (~24k) + `CLAUDE.md` (~6k) von jeder Lens erneut per `Read` geöffnet — obwohl der `@AGENTS.md`-Import sie ohnehin vollständig in jeden Agenten-Kontext legt. Drei Maßnahmen, ohne Befund-Verlust:
  ...
  ⚠️ **Was die Diät ausdrücklich nicht trifft:** die **Sabotage-/Red-Green-Prüfung** der Test-Lens. Sie ist der teuerste Schritt und der einzige, der harte Fehler statt Stil-Anmerkungen liefert — in der Messsession fand genau sie den einzigen echten Blocker und verifizierte in Runde 2 alle Fixes. Ein Wächter, der nur Ersparnis prüft, würde ihr Wegkürzen belohnen; darum nagelt [`test/review-context.test.ts`](../test/review-context.test.ts) sie als Zusicherung fest, neben Materialisierung, Diät und Frische-Guard. **Komplementär zu #1028** (senkt die *Größe* von AGENTS.md) — dieses Ticket senkt, *wie oft* derselbe Kontext beschafft wird; beide greifen multiplikativ.
...
- **Modellwahl: planen stark, umsetzen schnell (#741, #910).** Drei Tiers: **Plan + Review** → starkes Modell (Opus, hohes Reasoning), **Umsetzung** → Coding-Tier (Sonnet, per Alias statt Pin), **Explore/Recherche** → günstiges Modell (Haiku). **Wichtige Erkenntnis:** das ist **nicht** tool-übergreifend erzwingbar — welches Modell läuft, steuert immer der Harness/das Tool, keine Repo-Datei; eine fremde KI (Cursor/Codex/Gemini) liest `.claude/` gar nicht. Darum **zweischichtig** umgesetzt:
  ...
  - **Umgesetzt in #1035 (Stufe 3): auch die Umsetzung ist jetzt geroutet — auf beiden Pfaden.** Bis dahin galt der Coding-Tier nur dort, wo ein **Subagent** codet (Workflow: `agent({model:'sonnet'})`). Auf dem **Skill-Pfad** schreibt der **Hauptagent** den Code; er lief auf dem Session-Modell, aus einer Opus-Session also die komplette Umsetzung auf Opus — die Regel griff genau dort ins Leere, wo der maßgebliche, tool-neutrale Weg liegt. Gelöst mit denselben Frontmatter-Feldern wie bei `plan-feature`, nur am [`kubernia`-Skill](../.claude/skills/kubernia/SKILL.md) selbst (`model: sonnet`, `effort: medium`). **Die nötige Gegenmaßnahme:** weil dieser Override für den Rest des Turns gilt, hätte er den **Review** stillschweigend mitdemoviert — [`review-lenses`](../.claude/skills/review-lenses/SKILL.md) spawnt seine drei Lenses deshalb als eigene Opus-Subagenten statt inline; das macht aus der #1012-Verhaltensregel „kein Self-Grading" nebenbei eine Eigenschaft des Ablaufs. Bewacht von [`test/model-routing.test.ts`](../test/model-routing.test.ts) (Frontmatter-Hebel vorhanden, Lenses auf starkem Tier, Pin-Checkliste vollständig, keine Doku-Drift).
  - **Update-SSOT (#910):** Claude Code hat keine repo-eigenen Runtime-Aliases — aktuelle Modell-IDs + Checkliste, welche Dateien bei einem Modell-Wechsel anzupassen sind: [docs/model-routing.md](model-routing.md).
...
- **Human-in-the-Loop-Checkpoints für risikoreiche Diffs (#1012).** Der Marktstandard 2026 gate't nicht den ganzen Lauf, sondern die *riskante Entscheidung* („gate the decision, not the run") — und **Selbstmodifikation der Leitplanken** ist der Paradefall. Zwei Stopps ergänzen die obige Schleife: **(a) Pre-Flight-Klärung** — direkt nach Claim + Plan klassifiziert der Ablauf, ob das Ticket eine menschliche Entscheidung braucht (Harness-/Gate-Datei, 🎨 Optik, ⚠️ riskante Weiche, offene Plan-Weiche); trifft das zu, wird **vor dem Coden** geklärt (im `/kubernia`-Skill live per `AskUserQuestion`; im Workflow per **Halt → `resumeFromRunId`**, weil das Workflow-Tool kein Mid-Run-Ask-Primitiv hat — Auswahl + Plan kommen beim Resume aus dem Cache). **(b) Merge-Checkpoint** — fasst der Diff Harness-/Leitplanken-Dateien an (`AGENTS.md`, `CLAUDE.md`, `.claude/`, `.agents/`, `docs/agent-harness*` + die Goodhart-Gate-Config), **merged der Agent nicht selbst**, sondern übergibt an die Maintainerin (PR offen + CI grün; `maintainer-approved` + Merge bleiben menschlich). Maschinell: der `gate-change-guard` (§3) deckt jetzt auch die Leitplanken-Dateien ab (rot ohne Label), und [`test/harness-approval.test.ts`](../test/harness-approval.test.ts) hält die zwei Durchsetzungs-Listen (CODEOWNERS ⇄ PROTECTED) synchron. **Ehrliche Grenze (wie beim Goodhart-Guard):** im Single-Account ist das Label selbst setzbar — der echte menschliche Schritt ist der **Nicht-Self-Merge**. Zweischichtig wie die Modellwahl: portable Regel in [AGENTS.md](../AGENTS.md), Claude-Code-Automatik additiv im Workflow.
- **Review ohne Self-Grading (#1012).** Der agentische Mehr-Perspektiven-Review (`review-lenses`, §2.5) läuft im Ticket-Ablauf jetzt als **beschränkte review↔fix-Konvergenzschleife**: ein *frischer*, unabhängiger Kritiker pro Runde beurteilt den aktuellen Diff (Generator-Critic-Trennung), Ausstieg bei null blockierenden Findings, **Cap 2** Runden (unbeschränktes Iterieren ist schlechter, nicht besser — jenseits echter Fehler werden Stil-Nörgeleien erfunden), sonst Hand-off. So ist der finale „OK"-Blick nie der Agent, der zuletzt gefixt hat. Grounding bleibt `npm run verify` (rot ⇒ kein Lens-Pass).
...
**Zuletzt gelandet (#1012):** die **Human-in-the-Loop-Checkpoints** (§4, Wo die Grenzen sind) — Pre-Flight-Klärung vor dem Coden + Merge-Hand-off für Harness-/Leitplanken-Diffs (kein Self-Merge), plus der erzwungene **Review als Konvergenzschleife** ohne Self-Grading. Portable Regeln in AGENTS.md; der `gate-change-guard` deckt jetzt auch die Leitplanken-Dateien ab ([`test/harness-approval.test.ts`](../test/harness-approval.test.ts) hält CODEOWNERS ⇄ PROTECTED synchron); die Claude-Code-Automatik (Halt→Resume, Merge-Checkpoint) sitzt additiv im Workflow.
