...
> **Stand: 2026-06-26 — nächstes Ticket: #166. Kopf: 8 Einträge. Phase 10 (Heimat-Werft) läuft: #164 (Sim-Grundlage) + #165 (Werft-Region) erledigt, als nächstes #166 (Werft-NPC: Sprite + Smalltalk). Zuletzt abgeschlossen: #165 (Heimat-Werft-Region: neue datengetriebene Region als Phase-10-Capstone-Bühne — pures Modul `src/werft.ts` wie flotte.ts (Werft-Hof aus Stein-Kai + Gras, hölzerne Helling/Slipway im Süden, Anleger/Warp von Port Kubernia an der freien Kai-Lücke x22–23 zwischen Archipel- und Wachturm-Anleger), RegionConfig + `decorate`-Hook in scenes/regions.ts (das im Bau befindliche Schiff aus dem gemeinsamen ship-Sprite + zwei prozedurale Bau-Gerüste + Funken-Andeutung; sparsame Begrünung), REGION_WARPS-Eintrag + weltseitiger Anleger/Schild „Zur Werft" in terrain.ts; NPC-/Quest-Trigger-Standplatz begehbar reserviert für #166/#167; 14 neue Tests test/werft.test.ts Red-Green, warps.test.ts auf 6 Regionen, npm test 1108 grün, typecheck/lint/check:arch/check:size + Browser grün, CLAUDE.md-Landkarte um werft.ts + nachgezogene flotte.ts-Zeile ergänzt; Optik-Folgeticket #456 für echtes Werft-Gantry/Schiff-im-Bau-Asset). Davor #164 (Werft-Capstone-Sim-Grundlage: der ganze Ablauf eigenen Service bauen→deployen→erreichbar ist spielbar — neuer Welt-Befehl `curl [http://]<svc>[:port][/pfad]` (in `src/sim/net.ts`, zusammen mit dem dorthin ausgelagerten `nslookup`, da sim.ts sonst das 800-LOC-Budget sprengte), HTTP 200 bei Erreichbarkeit; drei Troubleshooting-Haken alle über `curl` als „Connection refused"+Tipp sichtbar: Image fehlt → ImagePullBackOff via Manifest-Flag `requireBuiltImage` (heilt selbst nach `docker build`/`rollout restart` über `needsBuild`, klassischer Tippfehler-ImagePull unberührt), falscher Port in der URL, targetPort≠containerPort (Endpoints da, curl läuft ins Leere); `targetPort`/`containerPort` optional+rückwärtskompatibel (auch `expose --target-port`), Snapshot-Round-Trip, `get endpoints` zeigt targetPort; 16 neue Tests `test/sim/werft.test.ts` Red-Green, npm test 1094 grün, typecheck/lint/check:arch/check:size/Boot-Smoke grün, Doku CLAUDE.md+docs/module/sim.md). Phase 9 (Expeditions-Flotte) ist komplett (#149–#157); Phase 10 (Heimat-Werft, #164→#165→#166→#167→#169→#171→#172) ist der laufende Content-Arc.**
...
|---|--------|------|--------------|---------------------------|
| | **— Heimat-Werft (Phase 10: Capstone — eigenen Service bauen → deployen → erreichbar) —** | | | Aus dem Reaktivierungs-Pool geholt (2026-06-26), nachdem Phase 9 komplett war. Dependency-geordnet wie Phase 9: erst Sim-Grundlage, dann Welt/NPC, dann Quest, dann Drills/Progression, Tests zuletzt. (#164/#168/#170 bereits erledigt.) |
| 1 | **#166** | niedrig | Werft: neuer NPC (Sprite + Smalltalk) | **Nächstes Ticket.** Steht in der Werft-Region (#165 ✓). ⚠️ Sprite = Grafik: PixelLab + Stardew-Referenz. |
| 2 | **#167** | niedrig | Werft: Quest „Eigenen Service containerisieren & deployen" | Braucht Sim (#164 ✓), Region (#165 ✓) und NPC-Geber (#166). Die Sim-Bausteine (`curl`, `requireBuiltImage`/ImagePullBackOff, targetPort/containerPort) stehen seit #164 bereit. |
| 3 | **#169** | niedrig | Werft: Drills (Capstone-Übungen) + PRACTICE-Mapping | Baut auf der Quest (#167) auf — dieselben Befehle als freies Üben. |
| 4 | **#171** | niedrig | Werft: Progression einhängen + README-Phasentabelle (Phase 10) | Nach Quest/Drills — hängt den Arc in den Lernpfad + zieht die README nach. |
| 5 | **#172** | niedrig | Werft: Tests für Capstone-Quests & Werft-Sim | **Zuletzt im Arc** — sichert ab (Red-Green), wie #157 für Phase 9. |
| | **— QoL / System-Features —** | | | |
