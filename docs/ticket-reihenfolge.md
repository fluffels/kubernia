...
> **Stand: 2026-06-26 — nächstes Ticket: #169. Kopf: 6 Einträge. Phase 10 (Heimat-Werft) läuft: #164 (Sim-Grundlage) + #165 (Werft-Region) + #166 (Werft-NPC Greta) + #167 (Capstone-Quest) erledigt, als nächstes #169 (Werft-Drills + PRACTICE-Mapping). Zuletzt abgeschlossen: #167 (Capstone-Quest „Eigenen Service containerisieren & deployen" — neue Quest `werft-eigener-dienst` bei Greta in `src/content/data/quests/greta.json`: 10 Schritte (dialog/terminal/teach/choice mit reviewId), inline-Scenario mit Dockerfile/server.js/deployment.yaml/service.yaml; fährt den ganzen Bogen Code→Image→Deployment→Service→erreichbar inkl. authentischem ImagePullBackOff→`docker build`→`rollout restart`-Loop (nutzt die Werft-Sim #164: requireBuiltImage, containerPort/targetPort, `curl`); `scenario.podSecurity: privileged`, damit das `apply` robust gegen vorher gesetzte Pod-Security-Stufen ist; führt `curl` als neuen Befehl per teach-Schritt ein (Klopftest → 200). Neues Logbuch-Thema `capstone` (quest-topics.json), quest-order ans Ende, 3 Review-Quiz-Karten in crabquiz/capstone.json; Langform-zuerst (#380/#286): --filename/--tag in Lösungen+Texten, accept beide Formen; README-Quest-Zahl 51→52 + Phase-10-Eintrag; Save-Migrations-Fixture savegame-v2-allquests.json nachgezogen. npm test 1109 grün, typecheck/lint/check:arch/check:size + Offline-Boot-Smoke grün; live verifiziert (Quest in Roadmap + HUD-Ziel + Werft-Szene mit Greta)). Davor #166 (Werftmeisterin Greta als Capstone-Mentorin: Sprite `char_greta`, npcs/smalltalk/entities ergänzt, Entity-Registry-Spawn) und #165 (Heimat-Werft-Region: pures Modul `src/werft.ts`, RegionConfig + Anleger/Warp). Phase 9 (Expeditions-Flotte) ist komplett (#149–#157); Phase 10 (Heimat-Werft, #164→#165→#166→#167→#169→#171→#172) ist der laufende Content-Arc.**
...
|---|--------|------|--------------|---------------------------|
| | **— Heimat-Werft (Phase 10: Capstone — eigenen Service bauen → deployen → erreichbar) —** | | | Aus dem Reaktivierungs-Pool geholt (2026-06-26), nachdem Phase 9 komplett war. Dependency-geordnet wie Phase 9: erst Sim-Grundlage, dann Welt/NPC, dann Quest, dann Drills/Progression, Tests zuletzt. (#164/#165/#166/#167/#168/#170 bereits erledigt.) |
| 1 | **#169** | niedrig | Werft: Drills (Capstone-Übungen) + PRACTICE-Mapping | **Nächstes Ticket.** Baut auf der Quest (#167 ✓) auf — dieselben Befehle (docker build, apply, rollout restart, curl) als freies Üben. |
| 2 | **#171** | niedrig | Werft: Progression einhängen + README-Phasentabelle (Phase 10) | Nach Quest/Drills — hängt den Arc in den Lernpfad + baut die README-Phasentabelle aus (Quest-Zahl 52 + Phase-10-Eintrag stehen seit #167 schon). |
| 3 | **#172** | niedrig | Werft: Tests für Capstone-Quests & Werft-Sim | **Zuletzt im Arc** — sichert ab (Red-Green), wie #157 für Phase 9. |
| | **— QoL / System-Features —** | | | |
| 4 | **#334** | niedrig | Dev-Panel per Docker, Passwort zur Laufzeit | Niedrige Dringlichkeit; baut auf #325/#331. |
