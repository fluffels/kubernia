...
|---|--------|------|--------------|---------------------------|
| | **— Tiefer Lernpfad: Aufbau-Bogen (#239 aufgeteilt, abhängige Kette) —** | | | |
| 1 | **#460** | niedrig | Sim-Fundament: leerer/zerstörter Cluster + `kubeadm init`/`join` | **Basis des Aufbau-Bogens #239 — zuerst.** Pure Domäne, ermöglicht alle Aufbau-Quests. |
| 2 | **#461** | niedrig | Einstiegs-Quest: der große Sturm zerstört den Hafen-Cluster | Narrativer Einstieg; nutzt #460 + vorhandene Sturm-Mechanik. |
| 3 | **#462** | niedrig | Quest: Control-Plane aufbauen (`kubeadm init`) | Nutzt #460; folgt der Sturm-Quest. |
| 4 | **#463** | niedrig | Quest: Worker-Knoten anschließen (`kubeadm join`, Node für Node) | Nach #462. |
| 5 | **#464** | niedrig | Quest: Dienste wieder ausbringen (Workloads Pod für Pod) | Nach #463; vorhandene apply-Mechanik. |
| 6 | **#465** | niedrig | Capstone: Cluster als Code wieder aufbauen (Terraform-Provisioning) | Pointe manuell ↔ als Code; nutzt #460 + Terraform-Familie. |
| 7 | **#466** | niedrig | Crab-Quiz: Bootstrapping-Konzepte vertiefen (#281/#282-Muster) | Braucht ein chapter aus den Aufbau-Quests. |
...
| 9 | **#278** | niedrig | Sammelalbum/Glossar: alle Befehle & Wissen entdecken und sammeln | Content/Lernpfad, design-frei; sammelt vorhandenes Wissen, kein neues Asset. |
| 10 | **#328** | niedrig | Lernthema Sandbox / ephemere Umgebungen: gefahrlos ausprobieren | Content/Lernpfad, design-frei; nutzt das vorhandene Wiederspiel-Feature (#326/#332) als lebendes Beispiel. |
| | **— QoL / Didaktik (design-frei) —** | | | |
| 11 | **#358** | ohne | help zeigt nur freigeschaltete Befehle (zu Spielbeginn: keine/nur help) | UX/Didaktik, design-frei; eng mit #359 (beide am help-Befehl). |
| 12 | **#359** | ohne | help-Ausgabe: console-typisches Format (ein Befehl pro Zeile) | UX, design-frei; gehört zu #358. |
| 13 | **#362** | ohne | Freies Funken: nach Befehlsausführung Erklärung einblenden | Didaktik, design-frei; vertieft das freie Funken-Terminal. |
| 14 | **#363** | ohne | Freies Funken: echte Tool-Namen/Images durch spielweltinterne Begriffe ersetzen | Content/Konsistenz, design-frei; gehört zu #362. |
...
| 16 | **#459** | niedrig | kubectl-secrets-Pseudo-Abkürzung überdenken (secret keine echte Kontraktion) | Tech-/Pädagogik-Debt, design-frei; direkte #430-Folge, kleiner Konsistenz-Fix. |
| 17 | **#382** | niedrig | Doku-Inkonsistenz: Worktree-Pfad-Konvention (AGENTS.md vs CLAUDE.md vs launch.json) | Tech-Debt/Doku, design-frei; klärt eine widersprüchliche Konvention. |
| | **— Anlegende / Sonderfälle ans Ende —** | | | |
| 18 | **#277** | niedrig | Ideen-Ticket: weitere Minispiele überlegen & dafür Tickets anlegen | Anlegend, design-frei; erzeugt Folge-Tickets statt direktem Fix. |
| 19 | **#314** ⚠️ | niedrig | Zentrales Feier-Popup-System (Konfetti + Spruch) | **Optik-Ticket: erst Vorstellung + Referenzbilder mit der Maintainerin abstimmen** (übergreift #223). |
| 20 | **#293** ⚠️ | niedrig | Spiellogik-Review (anlegend) | **ZULETZT** — erst wenn der Backlog weitgehend leer ist (sonst veraltet das Review sofort). Erzeugt Folge-Tickets, kein direkter Fix. |
