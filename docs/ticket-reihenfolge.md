...
> **Stand: 2026-06-27 — nächstes freies Ticket: #212 (Inhalt: erklären, was nginx ist – Webserver – an passender Stelle in der Lernstory; thematische Fortsetzung des gerade abgeschlossenen Image-Erklär-Arcs). Kopf: #212 + ⚠️-Sonderfälle. Zuletzt abgeschlossen: #450 (neue Bo-Quest `docker-rabbitmq` „Die Posthalle am Kai" an Index 8, ans Ende des Docker-Images-Arcs nach `docker-registry` — RabbitMQ = Message Broker: Dienste legen Nachrichten in eine Queue/Warteschlange, andere holen sie später ab → entkoppelte, asynchrone Kommunikation; Einsatz für Hintergrundaufgaben & Events zwischen (Micro-)Services; `docker pull/run rabbitmq` in Erklär-Kontext eingebettet; Drill `docker-run-rabbitmq` + 3 Quiz-Karten q-rabbit-1..3; docker-registry-Abschluss teasert die Posthalle, Ole-Handoff ans Quest-Ende verschoben; abgeleitete questIdx-Erwartungen der Save-Migrations-Tests um den vierten Docker-Einschub angepasst — IDs bleiben stabil, #353). Davor: #449 (Bo-Quest `docker-registry` „Woher die Baupläne kommen"), #448 (Bo-Quest `docker-common-images` „Warenkunde: BusyBox, Redis, Postgres"), #272 (kubectl-Context-Quiz), #275 (Secret-Typen-Quiz; dabei Pre-existing-Bug #458 entdeckt — HTML-Tags in Quiz-Optionen via `esc()` als Literaltext), #274 (Job/CronJob-Quiz), #273 (Helm-Templates), #334 (Dev-Panel-Docker), #268 (Gateway-API-Quiz), #266 (DaemonSet/StatefulSet-Quiz), #265 (CRD/Operator-Quiz). npm test 1206 / typecheck / lint / arch / size grün. Phase 9 + 10 komplett.**
...
|---|--------|------|--------------|---------------------------|
| | **— Content-Arc: Images im Lernpfad erklären (dependency-geordnet) —** | | | Direkte Fortsetzung des Docker-Images-Arcs (#448 Warenkunde / #449 Registry / #450 RabbitMQ): Images, die Lernende begegnen, an passender Stelle erklären. |
| 1 | **#212** | niedrig | Inhalt: erklären, was nginx ist (Webserver) – an passender Stelle in der Lernstory | nginx ist das allererste Image, das Lernende ziehen/starten; gehört wie BusyBox/Redis/Postgres/RabbitMQ in die „Warenkunde". |
