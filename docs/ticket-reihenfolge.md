...
|---|--------|------|--------------|---------------------------|
| | **— Expeditions-Flotte (Phase 9: Terraform-Module / Remote State / Provider / Variablen & Outputs) —** | | | Sim-Grundlage #146 ✓, Konfig-Inhalte #147 ✓ (`data/terraform/flotte.json`), Region/Anleger/Warp #148 ✓ (`src/flotte.ts`, RegionScene-Config „Flotte"), NPC Saga #149 ✓, Quest 1 #150 ✓ (`terraform-modul`), Quest 2 #151 ✓ (`terraform-remote-state`), Quest 3 #152 ✓ (`terraform-provider`), Quest 4 #153 ✓ (`terraform-variablen-outputs`) — **Quest-Bogen komplett**. Verbleibend: Drills → Quiz → Progression → Tests. |
| 1 | **#154** | ohne | Drills (Module/Remote-State/Provider/Variablen-Übungen) + PRACTICE-Mapping | **Nächstes Ticket.** Nach den Quests — übt das Gelernte. |
| 2 | **#155** | ohne | Quiz-Karten (Terraform-Module, Remote State, Provider, Variablen & Outputs) | Wissens-Sicherung zur Region (vertieft die In-Quest-Review-Karten). |
| 3 | **#156** | ohne | Progression einhängen + README-Phasentabelle aktualisieren | Verdrahtet die Region in den Lernpfad. |
| 4 | **#157** | ohne | Tests für Terraform-Aufbau-Quests & Modul/Remote-State/Provider-Sim | Zuletzt im Arc — sichert ab (Red-Green). |
| | **— QoL / System-Features —** | | | |
