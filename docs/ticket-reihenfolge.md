...
> **Stand: 2026-06-26 — nächstes Ticket: #172. Kopf: 4 Einträge. Phase 10 (Heimat-Werft) fast komplett: #164 (Sim-Grundlage) + #165 (Werft-Region) + #166 (Werft-NPC Greta) + #167 (Capstone-Quest) + #169 (Werft-Drills) + #171 (Progression/README-Phasentabelle) erledigt, als nächstes #172 (Tests für Capstone-Quests & Werft-Sim) als Arc-Abschluss. Zuletzt abgeschlossen: #171 (Phase 10 als „im Spiel" markiert — README-Phasentabelle um Zeile 10 „Heimat-Werft" ergänzt, Intro Phasen 1–9→1–10, Roadmap nachgezogen; analog #156 für Phase 9. Die Progression war bereits durch #164–#169 datengesteuert eingehängt: `quest-order.json` (`werft-eigener-dienst`), `entities.json` (Greta auf `werft`-Map), `REGION_WARPS` (`werft`); Story-Durchspiel-Tests grün, 52 Quests. README-only, kein Code. npm test 1109 / typecheck / lint / check:arch / check:size grün). Davor #169 (Capstone-Drills + PRACTICE-Mapping bei greta) und #167 (Capstone-Quest `werft-eigener-dienst`). Phase 9 (Expeditions-Flotte) ist komplett (#149–#157); Phase 10 (Heimat-Werft, #164→#165→#166→#167→#169→#171→#172) ist der laufende Content-Arc.**
