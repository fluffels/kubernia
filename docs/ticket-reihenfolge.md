...
> **Stand: 2026-06-26 — nächstes Ticket: #171. Kopf: 5 Einträge. Phase 10 (Heimat-Werft) läuft: #164 (Sim-Grundlage) + #165 (Werft-Region) + #166 (Werft-NPC Greta) + #167 (Capstone-Quest) + #169 (Werft-Drills) erledigt, als nächstes #171 (Progression einhängen + README-Phasentabelle). Zuletzt abgeschlossen: #169 (Capstone-Drills + PRACTICE-Mapping — 5 neue Drills in `src/content/drills.ts`, die die Capstone-Kette aus #167 als freies Üben spiegeln (NPC greta, freigeschaltet nach `werft-eigener-dienst`): `werft-build` (eigenes Image bauen), `werft-deploy-imagepull` (Deployment → ImagePullBackOff via Werft-Sim `requireBuiltImage` #164), `werft-rollout-heal` (nach dem Bauen `rollout restart`), `werft-expose` (Service davor, port 80→targetPort 8080), `werft-curl` (Klopftest → 200). Helfer `freeWerftName` zieht kollisionsfreie Bau-Image-Namen; jeder Drill mischt seinen Sandkasten additiv ein (`mergeScenario`/`applyEffects`); accept deckt Lang-/Kurzform ab (-f/--filename, -t/--tag). PRACTICE-Mapping bei greta ergänzt. drills.ts überschreitet damit das God-File-Budget (838>800 LOC) → Split-Ticket #457 angelegt + bis dahin in der check-size-ALLOWLIST dokumentiert geduldet. npm test 1109 grün, typecheck/lint/check:arch/check:size grün; live gegen die Laufzeit-Sim verifiziert (alle 5 fehlerfrei, curl→200, Gating greift). Davor #167 (Capstone-Quest `werft-eigener-dienst`) und #166 (Werftmeisterin Greta). Phase 9 (Expeditions-Flotte) ist komplett (#149–#157); Phase 10 (Heimat-Werft, #164→#165→#166→#167→#169→#171→#172) ist der laufende Content-Arc.**
...
|---|--------|------|--------------|---------------------------|
| | **— Heimat-Werft (Phase 10: Capstone — eigenen Service bauen → deployen → erreichbar) —** | | | Aus dem Reaktivierungs-Pool geholt (2026-06-26), nachdem Phase 9 komplett war. Dependency-geordnet wie Phase 9: erst Sim-Grundlage, dann Welt/NPC, dann Quest, dann Drills/Progression, Tests zuletzt. (#164/#165/#166/#167/#168/#169/#170 bereits erledigt.) |
| 1 | **#171** | niedrig | Werft: Progression einhängen + README-Phasentabelle (Phase 10) | **Nächstes Ticket.** Nach Quest/Drills — hängt den Arc in den Lernpfad + baut die README-Phasentabelle aus (Quest-Zahl 52 + Phase-10-Eintrag stehen seit #167 schon). |
| 2 | **#172** | niedrig | Werft: Tests für Capstone-Quests & Werft-Sim | **Zuletzt im Arc** — sichert ab (Red-Green), wie #157 für Phase 9. Deckt auch die Capstone-Drills aus #169 mit ab. |
| | **— QoL / System-Features —** | | | |
