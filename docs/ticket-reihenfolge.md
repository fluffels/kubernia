...
> **Stand: 2026-07-01 — zuletzt #479:** Erster DDD-Value-Object-Schritt — `PodName` (DNS-1123-Regel + `podName()`/`asPodName()`/`isResourceName()`) in neuem `src/sim/names.ts`, `PodInstance.name` gebrandet. Branded Type → kein Laufzeit-Overhead, serialisiert unverändert (Spielstände unberührt), kein sichtbares Verhalten geändert. Folge-Tickets: **#489** (Validierung an der Befehlsgrenze, bewusste Verhaltensänderung) + **#490** (Value Object Dublonen/Wirtschaft). 1348 Tests/typecheck/lint/arch/docmap grün.
> **Kuratierung 2026-07-01 (SIG-AI-Harness-Diskussion + arc42-Analyse, [arc42-architektur.md](arc42-architektur.md)):** Auf Maintainerin-Wunsch stehen **alle heutigen Architektur-/Harness-Tickets als ein Block ganz oben** — die DDD-Präzisierung #488 (getippte Aggregat-Methoden, Forts. #478) + die #479-Folge #489/#490, dazu #480 (Präsentations-Tests) und #481 (Barrierefreiheit). #477/#482 (Kontexte benennen + Doku↔Code-Wächter), **#478** (Aggregat-Invarianten-Grenze), **#483** (kontext-lokale Regeln pro Sub-Modul) und **#479** (Value Objects, erster Schritt) sind erledigt. **Nächstes freies Ticket = oberstes des Kopfes** (#488).
>
...
| 1 | **#488** | mittel | Workload-Mutationen über getippte Aggregat-Methoden (statt direktem Array-Zugriff), Forts. #478 | Folge aus #478 (Invarianten-Wächter steht); die Primitive-Grenzen aus #479 (erster Schritt) stehen. |
| 2 | **#489** | mittel | Ressourcen-Namen an der Befehlsgrenze validieren (Forts. #479) | Direkte Fortsetzung des #479-VO: die `podName()`-Regel an `kubectl create/apply` verdrahten (bewusste Verhaltensänderung → eigenes Ticket). |
| 3 | **#490** | mittel | Value Object für Dublonen/Wirtschaft (Forts. #479) | #479 nannte als nächsten Bereich ausdrücklich die Wirtschaft; Save-Format nicht brechen (#350). |
