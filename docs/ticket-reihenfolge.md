...
| H3 | **#527** | hoch | Aggregat-Kommando `npm run verify` (check:all) als eine SSOT für alle Gates | Baustein für #528; verhindert vergessene Gates + CI/lokal-Drift. |
| H4 | **#528** | hoch | Git pre-push-Hook — schnelle Gates lokal vor `git push origin main` | Schließt die einzige echte Netzlücke (Post-hoc-CI bei Direkt-Push). Nach #527. |
