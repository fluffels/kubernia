...
|---|--------|------|--------------|---------------------------|
| | **— Expeditions-Flotte (Phase 9: Terraform-Module / Remote State / Provider / Variablen & Outputs) —** | | | Sim-Grundlage #146 ✓, Konfig-Inhalte #147 ✓ (`data/terraform/flotte.json`), Region/Anleger/Warp #148 ✓ (`src/flotte.ts`, RegionScene-Config „Flotte"), NPC Saga #149 ✓, Quest 1–4 #150–#153 ✓ (Modul/Remote-State/Provider/Variablen-Outputs), Drills #154 ✓ (PRACTICE-Pool `saga`), Quiz #155 ✓ (8 vertiefende Karten). Verbleibend: Progression → Tests. |
| 1 | **#156** | ohne | Progression einhängen + README-Phasentabelle aktualisieren | **Nächstes Ticket.** Verdrahtet die Region in den Lernpfad. |
| 2 | **#157** | ohne | Tests für Terraform-Aufbau-Quests & Modul/Remote-State/Provider-Sim | Zuletzt im Arc — sichert ab (Red-Green). |
| | **— QoL / System-Features —** | | | |
