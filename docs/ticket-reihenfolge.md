...
> **Stand: 2026-06-27 — nächstes freies Ticket: #450 (RabbitMQ als Message Broker, `docker pull rabbitmq`; letzter Schritt des Docker-Images-Arcs). Kopf: #450 + Sonderfälle. Zuletzt abgeschlossen: #449 (neue Bo-Quest `docker-registry` „Woher die Baupläne kommen" an Index 7, ans Ende des Docker-Arcs nach `docker-build-image` — Registry = zentrales Image-Lager, Docker Hub als öffentliche Standard-Registry, voller Name `<registry>/<repository>:<tag>`, `:latest` ist Konvention NICHT „neueste Version", private Registries ghcr.io/Harbor; `docker pull` zeigt jetzt den wirklich gezogenen Tag + echte Quelle statt hart „latest"; 2 Drills/2 Karten/3 Quiz-Karten; Merge mit dem Parallel-Batch unten, alle Save-Migrations-Indizes neu berechnet — drei Einschübe (docker-common-images@2, docker-registry@7, helm-templates@28) verschieben die Folge-Quests). Davor (Parallel-Batch): #448 (Bo-Quest `docker-common-images` „Warenkunde: BusyBox, Redis, Postgres" an Index 2), #272 (kubectl-Context-Quiz), #275 (Secret-Typen-Quiz; dabei Pre-existing-Bug #458 entdeckt — HTML-Tags in Quiz-Optionen via `esc()` als Literaltext), #274 (Job/CronJob-Quiz), #273 (Helm-Templates), #334 (Dev-Panel-Docker), #268 (Gateway-API-Quiz), #266 (DaemonSet/StatefulSet-Quiz), #265 (CRD/Operator-Quiz). npm test 1206 / typecheck / lint / arch / size grün. Phase 9 + 10 komplett.**
...
|---|--------|------|--------------|---------------------------|
| | **— Content-Arc: Docker-Images & Registry (dependency-geordnet) —** | | | Letzter Schritt des Docker-Images-Arcs nach #448 (Warenkunde) und #449 (Registry). |
| 1 | **#450** | niedrig | Quest/Inhalt: RabbitMQ als Message Broker (`docker pull rabbitmq`) | Konkreter Image-/Broker-Anwendungsfall nach #448/#449. |
