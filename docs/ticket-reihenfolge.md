...
|---|--------|------|--------------|---------------------------|
| | **— Expeditions-Flotte (Phase 9: Terraform-Module / Remote State / Provider) —** | | | Sim-Grundlage #146 ✓, Konfig-Inhalte #147 ✓ (`data/terraform/flotte.json`), Region/Anleger/Warp #148 ✓ (`src/flotte.ts`, RegionScene-Config „Flotte"), NPC Saga #149 ✓, Quest 1 #150 ✓ (`terraform-modul`), Quest 2 #151 ✓ (`terraform-remote-state`), Quest 3 #152 ✓ (`terraform-provider`). Reihenfolge: Quests → Drills → Quiz → Progression → Tests. |
| 1 | **#153** | ohne | Quest „Variablen & Outputs — Konfiguration sauber durchreichen" | **Nächstes Ticket.** Letzte Quest des Arcs; Konfig `flotte-variablen-outputs` (#147), Geberin Saga. |
| 2 | **#154** | ohne | Drills (Module/Remote-State/Provider-Übungen) + PRACTICE-Mapping | Nach den Quests — übt das Gelernte. |
| 3 | **#155** | ohne | Quiz-Karten (Terraform-Module, Remote State, Provider) | Wissens-Sicherung zur Region (vertieft die In-Quest-Review-Karten). |
| 4 | **#156** | ohne | Progression einhängen + README-Phasentabelle aktualisieren | Verdrahtet die Region in den Lernpfad. |
| 5 | **#157** | ohne | Tests für Terraform-Aufbau-Quests & Modul/Remote-State/Provider-Sim | Zuletzt im Arc — sichert ab (Red-Green). |
| | **— QoL / System-Features —** | | | |
