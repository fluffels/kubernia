...
|---|--------|------|--------------|---------------------------|
| | **━━━ iSAQB-Architektur-Analyse 2026-07-01 — HOCH (echte Bugs / fehlende Kern-Gates) ━━━** | | | |
| A5 | **#496** | hoch | Typsicherheit: `WorldSceneLike = any` → `WorldSceneFields`-Interface | Hebelt Typprüfung aller 6 Präsentations-Systemmodule aus. |
| | **━━━ MITTEL (Struktur-, Testbarkeits- & Governance-Präzisierung) ━━━** | | | |
| A6 | **#497** | mittel | Save: fehlgeschlagener `save()` bleibt unsichtbar → Fehler an Toast/Runtime-Sink | Stiller Fortschrittsverlust im localStorage-Fallback. |
| A7 | **#499** | mittel | Sim: reset/merge/snapshot-Triplizierung + `kubectlApply`-Monolith → Ressourcen-Registry | Größter Wartungs-Hebel der Domäne. |
| A8 | **#501** | mittel | Domänen-Ticks aus Szenen-`update()` → szenen-neutraler Taktgeber (economyTick-Bug) | Verifizierter Konsistenz-Bug (RegionScene). |
| A9 | **#500** | mittel | Präsentation: Spiel-/Bewertungslogik aus DOM-Methoden herauslösen (testbar) | Kernbewertung nur e2e-, nicht unit-testbar. |
| A10 | **#512** | mittel | Präsentation: `events.ts` (spielentscheidend) testbar machen → `resolveHazardTick`-Kern | 0 Tests auf Belohnung/Strafe. |
| A11 | **#507** | mittel | Sim: DNS-1123 Value Object → `ResourceName` an allen Namensgrenzen erzwingen | VO verspricht Garantie, die real kaum existiert. |
