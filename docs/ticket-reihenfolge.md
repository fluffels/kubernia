...
> **Stand: 2026-07-01 — zuletzt #481:** Barrierefreiheit-Audit (audit-first) → [`barrierefreiheit-audit.md`](barrierefreiheit-audit.md). Drei Dimensionen geprüft: **Tastatur** vollständig bedienbar (kein Ticket), **Kontraste** flächendeckend WCAG AA erfüllt (kein Ticket — der frühere „Fehler"-Verdacht rechnete gegen die per Cascade überschriebenen alten `:root`-Werte + verwechselte AAA mit AA), **Farb-Status** wenige echte Lücken (Quiz ✓/✗, Docker-Fässer laufend/gestoppt) → Folge-Ticket **#491** (Auto-Rest). Reine Doku-Änderung.
...
| 1 | **#484** | mittel | Lernlücke: Scheduler platziert blind bei request=0 — Eviction-Loop vs. Pending | Neues Lernpfad-Ticket (mittel), echte K8s-Scheduling-Tiefe; steht über den niedrig-QoL/Optik-Tickets (Content vor QoL). |
| | **— System / QoL —** | | | |
