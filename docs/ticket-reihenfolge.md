...
> **Stand: 2026-06-26 — nächstes Ticket: #167. Kopf: 7 Einträge. Phase 10 (Heimat-Werft) läuft: #164 (Sim-Grundlage) + #165 (Werft-Region) + #166 (Werft-NPC Greta) erledigt, als nächstes #167 (Capstone-Quest „Eigenen Service containerisieren & deployen"). Zuletzt abgeschlossen: #166 (Werftmeisterin Greta — neuer Heimat-Werft-NPC als Capstone-Mentorin: PixelLab-Sprite `char_greta` (1b2dd719; resolute Bauleiterin im blauen Werft-Overall, Schutzbrille hochgeschoben, Handschuhe + Werkzeuggürtel; size 48 → auf 48² runtergerechnet wie der Cast) nach `assets/pixellab/` + assets-data.ts (`scene: "Werft"`, neuer AssetScene-Wert); npcs/smalltalk/entities um `greta` + 4 Capstone-Smalltalk-Zeilen ergänzt (docker build/image, ImagePullBackOff, curl/200, targetPort — resolut-trockener Ton); steht über die Entity-Registry auf dem in #165 reservierten Hof-Platz (Kachel 9/7), RegionScene spawnt automatisch via npcSpawnsForMap; Greta-Verdrahtungstest (Sprite+Smalltalk+Spawn == reservierte WERFT_NPC-Kachel) + assets.test.ts EXPECTED_LAZY um Werft erweitert; npm test 1109 grün, typecheck/lint/check:arch/check:size + Browser (Greta in der Werft) grün; veraltete „NPC folgt mit #166"-Kommentare in werft.ts/regions.ts nachgezogen). Davor #165 (Heimat-Werft-Region als Phase-10-Capstone-Bühne: pures Modul `src/werft.ts` wie flotte.ts, RegionConfig + `decorate`-Hook in scenes/regions.ts, Anleger/Warp von Port Kubernia, NPC-/Quest-Trigger-Platz reserviert) und #164 (Werft-Capstone-Sim-Grundlage: `curl` in `src/sim/net.ts`, `requireBuiltImage`/ImagePullBackOff, targetPort/containerPort). Phase 9 (Expeditions-Flotte) ist komplett (#149–#157); Phase 10 (Heimat-Werft, #164→#165→#166→#167→#169→#171→#172) ist der laufende Content-Arc.**
...
|---|--------|------|--------------|---------------------------|
| | **— Heimat-Werft (Phase 10: Capstone — eigenen Service bauen → deployen → erreichbar) —** | | | Aus dem Reaktivierungs-Pool geholt (2026-06-26), nachdem Phase 9 komplett war. Dependency-geordnet wie Phase 9: erst Sim-Grundlage, dann Welt/NPC, dann Quest, dann Drills/Progression, Tests zuletzt. (#164/#165/#166/#168/#170 bereits erledigt.) |
| 1 | **#167** | niedrig | Werft: Quest „Eigenen Service containerisieren & deployen" | **Nächstes Ticket.** Braucht Sim (#164 ✓), Region (#165 ✓) und NPC-Geber Greta (#166 ✓). Die Sim-Bausteine (`curl`, `requireBuiltImage`/ImagePullBackOff, targetPort/containerPort) stehen seit #164 bereit. |
| 2 | **#169** | niedrig | Werft: Drills (Capstone-Übungen) + PRACTICE-Mapping | Baut auf der Quest (#167) auf — dieselben Befehle als freies Üben. |
| 3 | **#171** | niedrig | Werft: Progression einhängen + README-Phasentabelle (Phase 10) | Nach Quest/Drills — hängt den Arc in den Lernpfad + zieht die README nach. |
| 4 | **#172** | niedrig | Werft: Tests für Capstone-Quests & Werft-Sim | **Zuletzt im Arc** — sichert ab (Red-Green), wie #157 für Phase 9. |
| | **— QoL / System-Features —** | | | |
