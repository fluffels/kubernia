...
`lighthouse` d132cfe0 · `watchtower` 43e86eb8 · `house_office` 4a65fc0a · `house_forge` 55deb6cb · `house_chart` 168d23b8 (neu generiert #844: `view: low top-down`, frontale Elevation statt 3/4-Perspektive, kleiner Holzkiosk mit Schleppdach + Steuerrad-Deko statt Turm/Kirchen-Look — Nachbesserung einer Kollision mit einer parallelen Session bei #794/#842)
...
`door_office_open` cb8ad3af · `door_forge_open` 404ee3ac · `door_chart_open` 6e6ca29d (Tür-Offen-Sprites, #840; `create_map_object`, `view: side`, `selective outline`, `detailed shading`; gleiche Dimensionen wie geschlossen: office 32×53, forge 38×58, chart 34×58. Selber Bogen/Rahmen wie das geschlossene Gegenstück, Türblatt nach rechts aufgeschwungen, dunkles Innenraum-Öffnung sichtbar. IDs auto-löschen nach 8h — nur die PNG-Dateien sind dauerhaft. Beim Betreten in `openDoorAndEnter` (`warps.ts`) via `sprite.setTexture` auf das offene Sprite getauscht, nach 180 ms Delay ruft `enterInterior` auf)
...
`shed` 1a801a53 (Lagerschuppen für gestoppte Container, #678; `high top-down`, 128×128, `selective outline`, `detailed shading`, `high detail`; offen-fronter Holzschuppen mit Satteldach + dunklem Eingangsbereich. Ersetzt in `scenes/worldscene/scenery.ts` das frühere `graphics()`-Primitiv in `renderContainerLager`; auf 0.75 skaliert → 96×96px, Origin unten-mitte, Fußlinie an `footY` für korrektes Depth-Sorting; gestoppte Container-Fässer stehen weiterhin vor dem offenen Eingang)
`pier_ruined` 85a7921a (Beschädigter Holzsteg für die drei Cluster-Knoten-Anleger, #691; `high top-down`, 48×112, `selective outline`, `detailed shading`, `high detail`; 3 Planken breit × 7 Planken lang, kaputte/morsche Holzdielen mit Lücken und Löchern, dunkles Wasser schimmert durch, Holzsplitter auf den verbliebenen Planken, umgeknickte Stützpfosten, Treibgut im Wasser. Deckungsgleich mit dem 3×7-Kachel-Fußabdruck der Cluster-Stege bei `PIER_XS` — ein Sprite für alle drei Anleger (ahoi-control, ahoi-worker-1, ahoi-worker-2). Noch ohne Sim-Verdrahtung; die Kopplung an den kubeadm-Zustand kommt in Teil 3–4 von #467)
`lighthouse_ruined` · `crane_wrecked` · `house_office_damaged` · `ship_wrecked` (Sturm-Trümmer-Varianten v2, #690; `muted warm earthy palette`, `soft dark-tinted outlines not pure black`, `single light source top-left`, `selective outline`, `detailed shading`, `high detail`; strukturell eingestürzt — obere Hälfte abgebrochen, umgekippter Kran, eingestürztes Dach, kenterndes Schiff. `lighthouse_ruined` 44×100, `side view`; `crane_wrecked` 160×176, `high top-down`; `house_office_damaged` 112×128, `low top-down` (Perspektive auf `house_office`-Stand gebracht, #838); `ship_wrecked` 288×176, `high top-down`. Reine Asset-Registrierung; Render-Logik (wann ruined statt original) folgt in Sturm-Optik 2/4+)
...
> **Schiff-Deck-Tiles (#760):** drei Assets für die begehbare Deck-Szene (#758). `deck_floor` (Holzplanken-Boden, `create_tiles_pro`, `square_topdown`, view_angle 90, `segmentation`-Mode → nahtlos, 16×16 RGBA PNG, Job `a8758ed0-9733-4d87-80c9-39634023b3b8`, Variante tile_15 — verwitterte Eichenplanken mit horizontaler Maserung, warme Erdtöne). `deck_reling` (Holzreling-Segment mit zwei Pfosten + Querholm, `create_map_object`, `high top-down`, `selective outline`, `detailed shading`, 64×64, transparenter Hintergrund). `deck_mast` (Mastfuß von oben, `create_map_object`, `high top-down`, `selective outline`, `detailed shading`, 32×32, transparenter Hintergrund, Tau/Takelung sichtbar). Alle drei folgen der Stardew-Palette (gedämpft-warm, `soft dark-tinted outlines not pure black`, `single light source top-left`). Scene-Integration folgt in #758.
...
> **Unterdeck-Tiles (#761):** zwei Assets für die Unterdeck-/Laderaum-Szene (Kind 2 aus #257, gerendert über `theme: "ship_underdeck"` in `scenes/InteriorScene.ts`, #759). `underdeck_floor` (dunkler, feuchter Laderaum-Holzboden – deutlich dunkler/holziger als das helle `interior_floor`/`deck_floor`; `create_tiles_pro`, `square_topdown`, view_angle 90, `segmentation`-Mode → nahtlos, 16×16 RGBA PNG, Job `84b24111-3650-4d96-b3ed-4746afc26778`, Variante tile_11 – verwitterte dunkle Eichenplanken, warme Erdtöne, ruhig/nahtlos). `hold_stairs` (hölzerner Niedergang/Treppe zurück aufs Deck mit Messing-Geländerandeutung, `create_map_object`, `view: high top-down`, 48², `selective outline`, `detailed shading`, `high detail`, transparenter Hintergrund). Beide in der gedämpft-warmen Stardew-Palette (Nordstern #44). In `InteriorScene.renderRoom` wählt `floorKey` fürs Unterdeck `underdeck_floor` statt `interior_floor`; `renderThreshold` setzt fürs Unterdeck `hold_stairs` als Austritt (statt der flachen `ship_hatch`-Luke, die auf dem Deck den Abstieg markiert). Deko bleibt bewusst bei den vorhandenen `barrel`/`crate` (minimaler Slice, Look mit der Maintainerin abgestimmt).
...
> **Gebäude** (`house_office` Stein/Hafenmeisterei 112×128, `house_forge` Werft 96×120, `house_chart` Kartenhaus 96×112; `view: low top-down`, `selective outline`, `detailed shading`, `high detail`; Stardew-Valley-Winkel ~45°, Fassade + Tür am unteren Bildrand klar sichtbar, #794): lösen die Kenney-„town"-Tile-Häuser ab. Gerendert über `building(x,y,w,tex,scale)` — Grundfläche (w×3 Tiles) bleibt solide, das Dach ragt nach oben, Tiefe nach Fußlinie (korrektes Vorne/Hinten zur Figur).
