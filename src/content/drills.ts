...
/* ---- Heimat-Werft (#164/#169, Phase 10): Capstone – den EIGENEN Dienst bauen → deployen →
 *  erreichbar machen. Diese Drills üben genau die Kette der Capstone-Quest (werft-eigener-
 *  dienst): aus dem Dockerfile ein eigenes Image bauen, es als Deployment ausrollen (und den
 *  ImagePullBackOff sehen, weil das Image noch nicht gebaut ist), nach dem Bauen per
 *  `rollout restart` nachziehen, einen Service davorlegen und mit `curl` den Klopftest
 *  machen. NPC: greta. Eigene Bau-Image-Namen (kein fremdes Registry-Image); jede Übung
 *  mischt ihren Sandkasten additiv ein (mergeScenario) bzw. legt Datei + Wirkung bereit –
 *  analog zu den ensure*-Helfern oben. */
const WERFT_NAMES = ["kombuese-dienst", "lotsen-dienst", "kai-dienst", "anker-dienst", "funk-dienst", "moewen-dienst"];

/** Zieht einen Werft-Dienst-Namen, der in Deployments UND Services noch frei ist – so
 *  kollidiert eine Übung im geteilten Sim nicht mit einer früher gebauten (#164). */
function freeWerftName(sim: Sim): string {
...
export const DRILLS: Record<string, (sim: Sim) => DrillTask> = {
  ...
  // #449: eine bestimmte Version per Tag holen – nicht blind irgendeine Fassung.
  ...
  // #449: aus einer bestimmten (z.B. privaten) Registry holen statt still von Docker Hub.
  ...
  // Image-Warenkunde (#448): drei feste Images mit ihrer jeweiligen Aufgabe – damit
  // sitzt nicht nur der run-Befehl, sondern WOFÜR jede Kiste steht.
  ...
  // Heimat-Werft / Capstone (#169) – die ganze Kette als freies Üben. NPC: greta.
  "werft-build": sim => {
    ensureDockerfile(sim);
  ...
  "werft-deploy-imagepull": sim => {
    ensureBarePodAdmission(sim); // rohes Deployment ohne securityContext – Phase-6-Härtung würde es sonst abweisen (#444)
    ensureDockerfile(sim);
    const name = freeWerftName(sim);
  ...
  "werft-rollout-heal": sim => {
    const name = freeWerftName(sim);
    // Du hast dein Image schon gebaut, aber der Pod hängt noch im alten ImagePullBackOff.
    sim.mergeScenario({
  ...
  "werft-expose": sim => {
    const name = freeWerftName(sim);
    // Ein laufender Dienst, aber noch ohne Service davor.
    sim.mergeScenario({
      dockerImages: [name + ":1.0"],
  ...
  "werft-curl": sim => {
    const name = freeWerftName(sim);
    // Dienst läuft + Service korrekt verdrahtet → der Klopftest liefert 200.
    sim.mergeScenario({
      dockerImages: [name + ":1.0"],
      deployments: [{ name, image: name + ":1.0", replicas: 1, containerPort: 8080 }],
...
export const PRACTICE: Record<string, { drill: string; after: string }[]> = {
  bo:   [{ drill: "docker-pull", after: "docker-first-container" }, { drill: "docker-run", after: "docker-first-container" }, { drill: "docker-run-busybox", after: "docker-common-images" }, { drill: "docker-run-redis", after: "docker-common-images" }, { drill: "docker-run-postgres", after: "docker-common-images" }, { drill: "docker-ps", after: "docker-list-containers" }, { drill: "docker-stop", after: "docker-list-containers" }, { drill: "docker-ps-a", after: "docker-list-containers" }, { drill: "docker-run-named", after: "docker-run-options" }, { drill: "docker-build", after: "docker-build-image" }, { drill: "docker-tag", after: "docker-build-image" }, { drill: "docker-pull-version", after: "docker-registry" }, { drill: "docker-pull-registry", after: "docker-registry" }],
